...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	activityCenterService := service.NewActivityCenterService(db, settingRepository)
	activityCenterHandler := admin.NewActivityCenterHandler(activityCenterService)
	broadcastService := service.NewBroadcastService(db, settingRepository)
	broadcastHandler := admin.NewBroadcastHandler(broadcastService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler, activityCenterHandler, broadcastHandler)
	...
	handlerActivityCenterHandler := handler.NewActivityCenterHandler(activityCenterService)
	handlerBroadcastHandler := handler.NewBroadcastHandler(broadcastService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, batchImageHandler, handlerActivityCenterHandler, handlerBroadcastHandler, idempotencyCoordinator, idempotencyCleanupService)
