...
import { listActivityCenter } from '@/api/activityCenter'
...
const modelPlazaEnabled = ref(false)
const activityCenterEnabled = ref(false)
const flagModelPlaza = () => modelPlazaEnabled.value
const flagActivityCenter = () => activityCenterEnabled.value

async function refreshSubNexusFeatures() {
  try {
    await getModelPlaza()
    modelPlazaEnabled.value = true
  } catch {
    modelPlazaEnabled.value = false
  }
  try {
    const result = await listActivityCenter()
    activityCenterEnabled.value = result.enabled
  } catch {
    activityCenterEnabled.value = false
  }
}

function handleSubNexusFeatureChanged() {
  void refreshSubNexusFeatures()
}
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/broadcasts', label: t('nav.broadcastsAdmin'), icon: BellIcon },
    { path: '/admin/activity-center', label: t('nav.activityCenterAdmin'), icon: GiftIcon, hideInSimpleMode: true },
    { path: '/admin/model-plaza', label: t('nav.modelPlazaAdmin'), icon: PriceTagIcon },
...
onMounted(() => {
  ...
  void refreshSubNexusFeatures()
  window.addEventListener('activity-center-config-changed', handleSubNexusFeatureChanged)
  window.addEventListener('model-plaza-config-changed', handleSubNexusFeatureChanged)
  if (isAdmin.value) {
    adminSettingsStore.fetch()
  }
...
onBeforeUnmount(() => {
  window.removeEventListener('activity-center-config-changed', handleSubNexusFeatureChanged)
  window.removeEventListener('model-plaza-config-changed', handleSubNexusFeatureChanged)
  if (sidebarNavRef.value) {
